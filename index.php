...
// Traer todos los productos para tener stock, precio e imagen
...
// Traer los productos más vendidos (sumando cantidad de la tabla compras)
$sqlPopulares = "
    SELECT p.ID, SUM(cantidad) AS total_vendido
    FROM compras c
    INNER JOIN productos p ON c.producto_id = p.ID
    GROUP BY p.ID
    ORDER BY total_vendido DESC
    LIMIT 6"; // Mostrar los 6 más vendidos

// Ejecuta la consulta SQL que está guardada en la variable $sqlPopulares
$resultadoPopulares = $conexion->query($sqlPopulares);
// Crea un array vacío donde se guardarán los productos más populares
$productosPopulares = [];
// Verifica que la consulta se haya ejecutado correctamente
if ($resultadoPopulares) {
  // Recorre cada fila de los resultados de la consulta
    while ($row = $resultadoPopulares->fetch_assoc()) {
        $id = $row['ID'];
         // Verifica si existe un producto con ese ID en el array $productos
        if(isset($productos[$id])) {
          // Si existe, agrega ese producto al array $productosPopulares
            $productosPopulares[] = $productos[$id];
        }
    }
}
?>
...
<div class="contenedor">
  ...
  <main class="comida">
    ...
    <div class="platos">
      <?php foreach($productosPopulares as $prod): ?>
      <article class="plato">
        <img src="Imagenes/<?php echo htmlspecialchars($prod['Imagen']); ?>" alt="<?php echo htmlspecialchars($prod['Nombre']); ?>" />
        <h1><?php echo htmlspecialchars($prod['Nombre']); ?></h1>
        <p>Deliciosa opción recomendada por nuestros clientes.</p>
